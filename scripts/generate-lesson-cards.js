...
/**
 * Script to generate lesson cards from lessons.json and save them to Firestore
 */
...
async function generateLessonCards() {
  try {
    console.log('📚 Generating lesson cards from lessons.json...');
    ...
    for (const stage of lessonsData) {
      ...
      for (const lesson of stage.lessons) {
        ...
        console.log(`  ✅ Created card: ${lesson.title}`);
      }
    }
    
    console.log(`\n📝 Generated ${lessonCards.length} lesson cards`);
    
    // Save to Firestore
    console.log('💾 Saving lesson cards to Firestore...');
    ...
    console.log(`\n🎉 Successfully saved ${savedCount}/${lessonCards.length} lesson cards to Firestore!`);
    console.log('📱 Now try the demo button - you should see lesson cards in the feed!');
    
  } catch (error) {
    console.error('❌ Error generating lesson cards:', error);
  } finally {
    process.exit(0);
  }
}
