...
import { WeeklyCompetitionDeck as WeeklyCompetitionDeckType, PredictionCard as PredictionCardType } from '../../types/deck';

const { width: screenWidth } = Dimensions.get('window');

interface WeeklyCompetitionDeckProps {
  deck: WeeklyCompetitionDeckType;
  onDeckComplete: (deckId: string) => void;
  onVote: (cardId: string, vote: 'up' | 'down') => void;
  onShare: (cardId: string) => void;
}

export const WeeklyCompetitionDeck: React.FC<WeeklyCompetitionDeckProps> = ({
  deck,
  onDeckComplete,
  onVote,
  onShare,
  ...
  const currentCard = deck.predictionCards[currentIndex];
  const isLastCard = currentIndex >= deck.predictionCards.length - 1;
  const isCompetitionActive = deck.status === 'active';
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.header}>
        <View style={styles.competitionInfo}>
          <Text style={[styles.competitionTitle, { color: theme.colors.onBackground }]}>
            {deck.title}
          </Text>
        ...
        <View style={styles.progressInfo}>
          <Text style={[styles.progressText, { color: theme.colors.onBackground }]}>
            {currentIndex + 1} of {deck.predictionCards.length}
          </Text>
            ...
            <View
              style={[
                styles.progressFill,
                {
                  ...
                  width: `${((currentIndex + 1) / deck.predictionCards.length) * 100}%`
                }
              ]}
            />
          </View>
      ...
      {/* Competition Details */}
      <View style={[styles.competitionDetails, { backgroundColor: theme.colors.surface }]}>
